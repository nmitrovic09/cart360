...
//first vibration motor pin
int pinVibA = 3;
//second vibration motor pin
int pinVibB = 5;
...
void loop()
{
  ...
  float temp = bme.readTemperature() / 100;
  float pressure = bme.readPressure() / 100;
  float humidity = bme.readHumidity() / 100;
  float gasResistance = bme.readGasResistance() / 100;
  float altitude = bme.readAltitude() / 100;
  float calibratedAltitude = bme.readCalibratedAltitude(seaLevel) / 100;
